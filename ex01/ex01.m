...
imshow(uint8(dimg_x_));
figure('Name','smoothed gradient y direction');
imshow(uint8(dimg_y_));
grad_n_ = (dimg_x_.^2+dimg_y_.^2).^0.5;
grad_d_ = atan2(dimg_y_, dimg_x_);
figure('Name','smoothed gradient magnitude');
imshow(uint8(grad_n_));
figure('Name','smoothed gradient orientation');
imshow(uint8((grad_d_+pi)*(255/(2*pi))));